...
# Select the GPS locations that are close to train tracks. TBD: Doesn't work.
buffer_50m <- tracks_line |> st_buffer(30)
#joined <- st_intersection(sf, st_as_sf(buffer))
#anzahl_in_buffer <- sum(!is.na(joined$...21))
#anzahl_nicht_in_buffer <- sum(is.na(joined$...21))

# Get the nearest point for each GPS location. 
nearest <- st_nearest_points(sf, tracks_line)
...
near_from <- dt_close_points[c(TRUE, FALSE)] # original GPS fixes
near_to <- dt_close_points[c(FALSE,TRUE)] # nearest track line points
```
